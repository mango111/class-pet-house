    ...
    <div class="bg-white rounded-2xl shadow-lg p-8 w-full max-w-md">
      ...
      <div class="space-y-4">
        <input v-model="username" type="text" placeholder="用户名"
          class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-pink-300 outline-none" />
        ...
        <button @click="handleReset" :disabled="loading"
          class="w-full py-3 bg-pink-400 hover:bg-pink-500 text-white rounded-xl font-medium transition active:scale-95">
          {{ loading ? '重置中...' : '重置密码' }}
        </button>
      </div>
      <div class="mt-4 text-center text-sm text-gray-400">
        <router-link to="/login" class="hover:text-pink-400">返回登录</router-link>
      </div>
    </div>
  </div>
</template>
